...
```{r Reg Focus Disrtibution, echo = FALSE,}
survey_completed %>%
...
survey_completed %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
The distribution seems mostly equal between genders, interesting that males seem to be promotion oriented at a level higher than women, relating to the findings in earlier research for disposition of women to care about public health issues at a higher rate
...
We´ve checked earlier if the message types had equal proportions of participants from different groups, but before we check if the different groups have seen the messges at a similar rate lets check the distribution of users between different groups ie different messaging treatment start date
...
When checking the interest rate of the messaging groups through the reate of seeing messages, we see that that the initial wave is the lowest, and the one From the first half of August stands out with a higher rate. We should note that the final 2 groups have not completed their treatment by the time of aquiring the data sets.
...
Checking the proportions by message seen category we notice that the initial responders had a very high rate of users that have not entered the application to read completly any of the messages.